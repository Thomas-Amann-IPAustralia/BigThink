...
| **Stage 1 checkpoints DuckDB around every frame** | The corpus travels between runs as the `.duckdb` file alone — a Release asset and an R2 mirror — and DuckDB leaves recent writes in a `.wal` that neither carries. A run killed at the job timeout never closes its connection, so nothing is folded in. The cancelled 2026-09-14 run collected 10,057 documents in six hours and published a corpus byte-identical in size to the one it started from, reporting success. `db.checkpoint()` bounds the loss to one frame. PROJECT_STATE issue 35 |
| **arXiv retires itself when the runner is throttled** | Per-year and per-frame containment (issue 14) each stopped a failure spreading, and together they turned a fast failure into a slow one: every arXiv frame cost ~20 minutes whether it returned 60 documents or none, and seven of eleven returned none — 64% of a six-hour run for 1% of the corpus. `retire_after_failed_frames` raises `PermanentError`, which is what Stage 1's existing circuit breaker retires a source on. A throttled IP is not a per-frame condition |
...
- **A new topic-level score** → add it to `BT.FIELDS` in
  `src/dashboard_assets/core.js`, with the one-line `why` that says what it
  means. That single table drives the Topics columns, the Scores axes and the
  map's continuous colouring, so a field added there appears in all three and
  cannot end up described differently in each.
